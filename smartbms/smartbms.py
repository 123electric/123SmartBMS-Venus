...
dbusservice.add_path('/DeviceInstance', 288+int(device_port_num))
...
dbusservice.add_path('/HardwareVersion', None)
dbusservice.add_path('/Serial', device_serial_numbers[args.device])
dbusservice.add_path('/Connected',     1)

# Create device list
dbusservice.add_path('/Devices/0/DeviceInstance',  0x288+int(device_port_num))
...
def update_values():
    ...
    BMS.lock.acquire()
    print('Pack voltage: {}'.format(BMS.pack_voltage))
    if(BMS.alarm_cell_communication or BMS.alarm_serial_communication):
        dbusservice["/Soc"] = None
        dbusservice["/SystemSwitch"] = None
        dbusservice["/Capacity"] = None
        dbusservice["/InstalledCapacity"] = None
        ...
        dbusservice["/Capacity"] = BMS.capacity
        dbusservice["/InstalledCapacity"] = BMS.capacity
        dbusservice["/Dc/0/Voltage"] = BMS.pack_voltage
        dbusservice["/Dc/0/Current"] = BMS.pack_current
        dbusservice["/Dc/0/Power"] = BMS.pack_voltage * BMS.pack_current
        dbusservice["/Dc/0/Temperature"] = BMS.highest_cell_temperature
        dbusservice["/Io/AllowToCharge"] = int(BMS.allowed_to_charge)
        dbusservice["/Io/AllowToDischarge"] = int(BMS.allowed_to_discharge)
        dbusservice["/System/MinCellVoltage"] = BMS.lowest_cell_voltage
        dbusservice["/System/MinVoltageCellId"] = BMS.lowest_cell_voltage_num
        dbusservice["/System/MaxCellVoltage"] = BMS.highest_cell_voltage
        dbusservice["/System/MaxVoltageCellId"] = BMS.highest_cell_voltage_num
        dbusservice["/System/MinCellTemperature"] = BMS.lowest_cell_temperature
        dbusservice["/System/MinTemperatureCellId"] = BMS.lowest_cell_temperature_num
        dbusservice["/System/MaxCellTemperature"] = BMS.highest_cell_temperature
        dbusservice["/System/MaxTemperatureCellId"] = BMS.highest_cell_temperature_num
        dbusservice["/System/NrOfModulesOnline"] = 1
        dbusservice["/System/NrOfModulesOffline"] = 0
        dbusservice["/System/NrOfModulesBlockingCharge"] = int(not BMS.allowed_to_charge)
        dbusservice["/System/NrOfModulesBlockingDischarge"] = int(not BMS.allowed_to_discharge)
        dbusservice["/Alarms/LowVoltage"] = int(BMS.alarm_minimum_voltage)
        dbusservice["/Alarms/HighVoltage"] = int(BMS.alarm_maximum_voltage)
        dbusservice["/Alarms/LowTemperature"] = int(BMS.alarm_minimum_temperature)
        dbusservice["/Alarms/HighTemperature"] = int(BMS.alarm_maximum_temperature)
        ...
        current_copy = BMS.pack_current
        if current_copy < 0:
            dbusservice['/TimeToGo'] = (BMS.soc*BMS.capacity * 10) * 60 * 60 / (BMS.pack_voltage * -1 * current_copy)
        else:
...
# Load the custom name from the persistent storage and send it to the dbus
settings = SettingsDevice(
        dbus.SessionBus() if 'DBUS_SESSION_BUS_ADDRESS' in os.environ else dbus.SystemBus(),
        supportedSettings={
        'CustomName': ['/Settings/123electric/Products/'+ device_serial_numbers[args.device] + '/CustomName', driver['name'], 0, 0]
        },
        eventCallback = handle_changed_setting)

def custom_name_changed(path, value):
    settings['CustomName'] = value
    return True
    
dbusservice.add_path('/CustomName', value=settings['CustomName'], writeable=True, onchangecallback=custom_name_changed)

BMS = SmartBMS(args.device)
